...
./run.sh [--no-dns] [-f <functest-config.yaml>] [-r|--reuse] [-l] [-v] [-k|--keep] [--dns-server <dns-server>] [--no-controller] [--old] [-- <options> <for> <ginkgo>]
...
Options:
    ...
    --old                  use old, non-compound controller
...
if [ "$RUN_CONTROLLER" == "true" ]; then
  if [ "$DNS_OLD" == "true" ]; then
    go build -mod=vendor -race -o $ROOTDIR/dns-controller-manager-old $ROOTDIR/cmd/olddns
    $ROOTDIR/dns-controller-manager-old --controllers=dnscontrollers,infoblox-dns --identifier=functest --omit-lease >/tmp/dnsmgr-functional.log 2>&1 &
    PID_CONTROLLER=$!
  else
  ...
  if [ "$DNS_OLD" == "true" ]; then
    echo dns-controller-manager-old must be started with arguments: '--controllers=dnscontrollers,infoblox-dns --identifier=functest'
  else
...
GOFLAGS="-mod=vendor" FUNCTEST_CONFIG=$FUNCTEST_CONFIG DNS_LOOKUP=$DNS_LOOKUP DNS_SERVER=$DNS_SERVER DNS_OLD=$DNS_OLD ginkgo -p "$@"
